...
export const login = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!user.subscription_level) {
      console.log(`LOGIN: User has no subscription_level set. Email: ${email}`);
    }
    ...
    const token = jwt.sign(
      {
        ...
        name: user.name,
        email: user.email,
        level: user.subscription_level || "curious",
      },
      JWT_SECRET,
      { expiresIn: '6h' }
    );
...
if (userLevel === undefined || accessPriority[userLevel] === undefined) {
  console.error("GET NEWS: Invalid or missing user level in token:", userLevel);
  res.status(403).json({ message: "Forbidden - invalid subscription level" });
  return;
}
...
// CANCEL SUBSCRIPTION -  Sets the subscription_level to "free"
export const cancelSubscription = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const [result] = await db.query("UPDATE users SET subscription_level = 'free' WHERE id = ?", [userId]);

    console.log("CANCEL SUBSCRIPTION: Update result:", result);

    // Skapa nytt token med level = "free"
    ...
    // Skicka tillbaka nytt token tillsammans med meddelande
    res.json({ message: "Subscription cancelled. You are now on the free plan.", token: newToken });
  } catch (err) {
